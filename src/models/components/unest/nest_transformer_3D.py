...
def blockify(x, block_size: int):
    ...
    x = x.permute(0, 1, 3, 5, 2, 4, 6, 7).reshape(B, grid_depth * grid_height * grid_width, -1, C) # shape [2, 512, 27, 128]
...
class NestTransformer3D(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed = PatchEmbed3D(
            img_size=[img_size, img_size, img_size], patch_size=[patch_size, patch_size, patch_size], in_chans=in_chans, embed_dim=embed_dims[0])
...
def _create_nest(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    model = build_model_with_cfg(
        NestTransformer3D, variant, pretrained,
